...
void DoTriggerInput()
{
  ...
  if ((millis() - lastTriggerDebounceTime) > debounceDelay)
  {
    if (reading != TriggerButtonState)  // check for change in trigger button state, either on->off or off->on
    {
      ...
      {        
        // if we're in continous flame mode or continous burst mode, then we stop when the user lets go of the button
        if(m_burst.Mode() == eContinousFlame || m_burst.Mode() == eContinousBurst)
        {
...
void CalibratePotInput()
{
  ...
  Serial.print("******** Analog solenoid potentiometer input calibration routine: ");

  Serial.print("Move the sensor to the MINIMUM position and press any key");
  while (Serial.available() > 0){} // do nothing
  pVars.solenoid_pot_min = analogRead(Solenoid_Input_Pin);
  
  Serial.print("Move the sensor to the MAXIMUM position and press any key");
  while (Serial.available() > 0){} // do nothing
